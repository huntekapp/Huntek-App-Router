...
const InterviewsHomepage = ({ interviewInfo, index }) => {
  console.log(index)
  return (
		<article className={`${index === 1 ? "hidden md:flex md:flex-row" : ""} flex flex-row w-full h-20 px-4 bg-sec text-xs xs:text-sm text-pri border rounded-xl shadow-md justify-between items-center mb-2`}>
			...
			<div className="w-[99%]">
				<span className="btn w-full shadow-sm bg-pri border-none text-sm xs:text-xl text-sec">
					<span>{interviewInfo.hour} | {interviewInfo.enterprise}</span>
